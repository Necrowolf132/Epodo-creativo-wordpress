...
class Hestia_Header extends Hestia_Abstract_Main {
	...
	private function navbar_header() {
		...
		<div class="navbar-header">
			<div class="title-logo-wrapper icon1" style="display: none">
				<a class="navbar-brand" href="<?php echo esc_url( home_url( '/' ) ); ?>"
						title="<?php bloginfo( 'name' ); ?>">
					<?php echo $this->logo(); ?></a>
			</div>
            <div class="title-logo-wrapper icon2">
                <a class="navbar-brand" href="<?php echo esc_url( home_url( '/' ) ); ?>"
                   title="<?php bloginfo( 'name' ); ?>">
                    <img src="<?php echo get_template_directory_uri() . '/assets/img/LogoMakr_3sOdwu2.png' ; ?>" alt="<?php echo esc_attr( $alt_attribute ) ?>"></a>
            </div>
			<?php
			if ( $this->is_full_screen_menu() ) {
				$this->navbar_sidebar();
