...
if ( ! class_exists( 'zerif_ourfocus' ) && zerif_check_if_old_version_of_theme() ) {
	...
	class zerif_ourfocus extends WP_Widget {
		...
		function update( $new_instance, $old_instance ) {
            ...
            $instance['image_uri2']           = esc_url( $new_instance['image_uri2'] );
            $instance['custom_media_id']     = sanitize_text_field( $new_instance['custom_media_id'] );
			$instance['custom_media_id2']     = sanitize_text_field( $new_instance['custom_media_id2'] );
			$instance['image_in_customizer'] = esc_url( $new_instance['image_in_customizer'] );
            $instance['image_in_customizer2'] = esc_url( $new_instance['image_in_customizer2'] );
		...
		function form( $instance ) {
			...
			// Imagen de targeta modificada
            echo '<p>';
            echo '<label for="' . $this->get_field_id( 'image_uri2' ) . '">' . 'Imagen de tarjeta animada'. '</label><br/>';

            $image_in_customizer2 = '';
            $display            = 'none';
            if ( ! empty( $instance['image_in_customizer2'] ) && ! empty( $instance['image_uri2'] ) ) {
                $image_in_customizer2 = esc_url( $instance['image_in_customizer2'] );
                $display             = 'inline-block';
            } else {
                if ( ! empty( $instance['image_uri2'] ) ) {
                    $image_in_customizer2 = esc_url( $instance['image_uri2'] );
                    $display             = 'inline-block';
                }
            }
            $zerif_image_in_customizer2 = $this->get_field_name( 'image_in_customizer2' );

            echo '<input type="hidden" class="custom_media_display_in_customizer" name="';
            if ( ! empty( $zerif_image_in_customizer2 ) ) {
                echo $zerif_image_in_customizer2;
            }
            echo '" value="';
            if ( ! empty( $instance['image_in_customizer2'] ) ) {
                echo $instance['image_in_customizer2'];
            }
            echo '">';
            echo '<img class="custom_media_image" src="' . $image_in_customizer2 . '" style="margin:0;padding:0;max-width:100px;float:left;display:' . $display . '" alt="' . __( 'Uploaded image', 'zerif-lite' ) . '"/><br/>';

            echo '<input type="text" class="widefat custom_media_url" name="' . $this->get_field_name( 'image_uri2' ) . '" id="' . $this->get_field_id( 'image_uri2' ) . '" value="';
            if ( ! empty( $instance['image_uri2'] ) ) {
                echo $instance['image_uri2'];
            }
            echo '" style="margin-top:5px;">';

            echo '<input type="button" class="button button-primary custom_media_button" id="custom_media_button" name="' . $this->get_field_name( 'image_uri2' ) . '" value="' . __( 'Upload Image', 'zerif-lite' ) . '" style="margin-top:5px;">';
            echo '</p>';
            ...
            echo '"/>';
            echo '<input class="custom_media_id" id="' . $this->get_field_id( 'custom_media_id2' ) . '" name="' . $this->get_field_name( 'custom_media_id2' ) . '" type="hidden" value="';
            if ( ! empty( $instance['custom_media_id2'] ) ) {
                echo $instance['custom_media_id2'];
            }
            echo '"/>';
		}
